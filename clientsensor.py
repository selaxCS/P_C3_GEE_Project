...
class sensor:
    ...
    def __init__(self):
        ...
        self.temp = random.uniform(-2,41)
        self.hum = random.uniform(0,100)
        self.flow = random.uniform(0,1)
        self.rain = random.uniform(0,10)
    ...
    def SensorAct(self):
        ...
        self.temp = max(0,min(40,self.temp+random.uniform(-0.1, 0.5)))
        
        self.hum = max(0, min(100, self.hum + random.uniform(-0.5, 0.5)))
        
        self.flow = max(0, self.flow + random.uniform(-0.05, 0.05))
        
        self.rain = max(0,min(1,self.rain + random.uniform(0, 0.02)))
...
def run_sender():
    ...
    while True:
        ...
        try:
            ...
            print("Error: Cannot connect to the receiver.")
